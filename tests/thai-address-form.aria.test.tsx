...
import { ThaiAddressForm } from '../templates/react/ts/shadcn/aria/thai-address-form'
...
// Same full-cascade selection pattern as tests/thai-address-cascade-select.aria.test.tsx's
// `pick`: the embedded cascade only resolves (and calls back with) a non-null selection once
// province, district, and sub-district are all picked, and each downstream trigger's native
// `disabled` property (RAC's SelectTrigger renders a real `<button>`) stays true until its own
// parent is chosen. Aria-specific by design; see the shared helper's `selectFullCascade` doc
// comment.
