...
**วิธีแก้:** คัดลอก block ทั้งหมดที่ `init` พิมพ์ไปไว้ใน global stylesheet ที่แอปโหลดจริง แล้วเก็บ path ที่ถูกต้องใน `tailwind.css` ของ [`thaizip.config.json`](/react-thai-zip/reference/config/) ถ้าจำเป็น
...
ดูคำสั่งและ flags ทั้งหมดได้ที่ [ข้อมูลอ้างอิง CLI](/react-thai-zip/reference/cli/)
